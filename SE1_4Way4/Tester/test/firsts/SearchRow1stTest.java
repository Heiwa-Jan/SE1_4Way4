...
class SearchRow1stTest {
	...
	void winnerTest() {
		...
		Player testPlayer = new Player("Player_1");
		testPlayer.setToken('X');

		for(int i = 0; i < 4; i++) testBoard = MoveAllTokens.move(testBoard, new Move(1, 2, 3, testBoard), 'X');//1bu

		//Test auf vertikal
		assertTrue(new SearchRow(testBoard, testPlayer).search());

		testBoard = new GameBoard(7, 7);
		for(int i = 0; i < 4; i++) testBoard = MoveAllTokens.move(testBoard, new Move(2, 1, 0, testBoard), 'X');//2al
