...
public class KI {
	...
	private final char kiToken;
	private final char opponentToken;
	// Bemerkung: kiToken=true, opponentToken=false = urspruenglich int
	// --> ???

	//blockierte Reihen(Gegner) werden stärker gewertet, als offene/geschlossene(KI)!
	...
	public Move randMove() {
		...
		boardDirection = MoveAllTokens.moveAll(this.getBoard(), currentMove);
		for(int i = 0; i < this.getBoard().getRows(); i++) {
			currentMove.setLine(i);
			if(currentMove.isValidMove()) {
				tempBoard = boardDirection.copy();
				tempBoard = MoveAllTokens.moveLast(tempBoard, currentMove, kiToken);
	 ...
	 * @return Länge der untersuchten Reihe
	...
	private static int row (GameBoard board, char token , int x, int y, int dx, int dy) {
		...
		//				&& ((feld.getField(x + 1 * dx, y + 1 * dy) == token) || (feld.getField(x + 1 * dx, y + 1 * dy) == ' '))
		//				&& ((feld.getField(x + 2 * dx, y + 2 * dy) == token) || (feld.getField(x + 2 * dx, y + 2 * dy) == ' '))
		//				&& ((feld.getField(x + 3 * dx, y + 3 * dy) == token) || (feld.getField(x + 3 * dx, y + 3 * dy) == ' '))) {
	...
	int rate(GameBoard tempBoard) {
		...
		for (int column = 0; column < WIDTH; column++) {
			for (int row = 0; row < HEIGHT; row++) {
				...
				if((HEIGHT - column > 4) && (WIDTH - row > 4)) {
					...
					else if (row(board, opponentToken, row, column, 1, 1) == 3) { //gegnerische 3er-Reihe nach rechts oben
				...
				if((HEIGHT - column > 3) && (WIDTH - row > 3)) {
					...
					else if (row(board, opponentToken, row, column, 1, 1) == 2) { //gegnerische 2er-Reihe nach rechts oben
				...
				if((row > 3) && (column < WIDTH-2)) {
					...
					else if (row(board, opponentToken, row, column, -1, 1) == 3) { // gegnerische 3er-Reihe nach rechts unten
				...
				if((row > 2) && (column < WIDTH-1)) {
					...
					else if (row(board, opponentToken, row, column, -1, 1) == 2) { //gegnerische 2er-Reihe nach rechts unten
		...
		return kiToken2FAKTOR * max2erOpen + opponentBlocked2FAKTOR * min2erBlocked + kiToken3FAKTOR * max3erOpen + opponentBlocked3FAKTOR * min3erBlocked - opponentToken2FAKTOR * min2erOpen - kiToken2FAKTOR * max2erBlocked - opponentToken3FAKTOR * min3erOpen - kiToken3FAKTOR * max3erBlocked;
	}
